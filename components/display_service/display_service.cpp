...
epaper_ui::SettingsAboutPageState s_settings_about_page_state = {};
...
struct RenderSnapshot {
    ...
    epaper_ui::SettingsAboutPageState settings_about_page = {};
...
const RenderSnapshot& CaptureRenderSnapshot()
{
    ...
    snapshot.settings_about_page = s_settings_about_page_state;
...
void DrawSettingsStorageUnderlay(uint8_t* framebuffer, const RenderSnapshot& snapshot)
{
    EpaperPanel& panel = Panel();
    panel.Clear(true);
    epaper_ui::DrawSettingsStoragePage(framebuffer,
                                       WAVESHARE_EPD_WIDTH,
                                       WAVESHARE_EPD_HEIGHT,
                                       kPortraitWidth,
                                       kPortraitHeight,
...
void DrawSettingsTodosUnderlay(uint8_t* framebuffer, const RenderSnapshot& snapshot)
{
    EpaperPanel& panel = Panel();
    panel.Clear(true);
    epaper_ui::DrawSettingsTodosPage(framebuffer,
                                     WAVESHARE_EPD_WIDTH,
                                     WAVESHARE_EPD_HEIGHT,
                                     kPortraitWidth,
                                     kPortraitHeight,
                                     snapshot.settings_todos_page,
                                     snapshot.status_bar,
                                     snapshot.global_footer);
}

void DrawSettingsTopicsUnderlay(uint8_t* framebuffer, const RenderSnapshot& snapshot)
{
    EpaperPanel& panel = Panel();
    panel.Clear(true);
    epaper_ui::DrawSettingsTopicsPage(framebuffer,
                                      WAVESHARE_EPD_WIDTH,
                                      WAVESHARE_EPD_HEIGHT,
                                      kPortraitWidth,
                                      kPortraitHeight,
                                      snapshot.settings_topics_page,
                                      snapshot.status_bar,
                                      snapshot.global_footer);
}

void DrawSettingsAboutUnderlay(uint8_t* framebuffer, const RenderSnapshot& snapshot)
{
    EpaperPanel& panel = Panel();
    panel.Clear(true);
    epaper_ui::DrawSettingsAboutPage(framebuffer,
                                     WAVESHARE_EPD_WIDTH,
                                     WAVESHARE_EPD_HEIGHT,
                                     kPortraitWidth,
                                     kPortraitHeight,
                                     snapshot.settings_about_page,
                                     snapshot.status_bar,
                                     snapshot.global_footer);
}
...
esp_err_t ApplySettingsStorage(RefreshMode refresh_mode)
{
    const RenderSnapshot& snapshot = CaptureRenderSnapshot();
    EpaperPanel& panel = Panel();
    DrawSettingsStorageUnderlay(panel.framebuffer(), snapshot);
    CaptureUnderlaySnapshot(panel.framebuffer());
    DrawCurrentOverlays(panel.framebuffer(), snapshot);
...
esp_err_t ApplySettingsTodos(RefreshMode refresh_mode)
{
    const RenderSnapshot& snapshot = CaptureRenderSnapshot();
    EpaperPanel& panel = Panel();
    ...
    s_current_screen.store(ScreenId::kSettingsTodos, std::memory_order_relaxed);
    RefreshBusyGuard refresh_busy;
    const esp_err_t err = RefreshForMode(panel, refresh_mode);
    if (err != ESP_OK) {
        return err;
...
esp_err_t ApplySettingsTopics(RefreshMode refresh_mode)
{
    const RenderSnapshot& snapshot = CaptureRenderSnapshot();
    EpaperPanel& panel = Panel();
    ...
    s_current_screen.store(ScreenId::kSettingsTopics, std::memory_order_relaxed);
    RefreshBusyGuard refresh_busy;
    const esp_err_t err = RefreshForMode(panel, refresh_mode);
    if (err != ESP_OK) {
        return err;
...
esp_err_t ApplySettingsAbout(RefreshMode refresh_mode)
{
    const RenderSnapshot& snapshot = CaptureRenderSnapshot();
    EpaperPanel& panel = Panel();
    DrawSettingsAboutUnderlay(panel.framebuffer(), snapshot);
    CaptureUnderlaySnapshot(panel.framebuffer());
    DrawCurrentOverlays(panel.framebuffer(), snapshot);

    s_current_screen.store(ScreenId::kSettingsAbout, std::memory_order_relaxed);
    RefreshBusyGuard refresh_busy;
    const esp_err_t err = RefreshForMode(panel, refresh_mode);
    if (err != ESP_OK) {
        return err;
...
esp_err_t RefreshCurrentScreenRegionLocked()
{
    ...
    switch (s_current_screen.load(std::memory_order_relaxed)) {
        ...
        case ScreenId::kSettings:
            DrawSettingsUnderlay(panel.framebuffer(), snapshot);
            break;
        case ScreenId::kSettingsStorage:
            DrawSettingsStorageUnderlay(panel.framebuffer(), snapshot);
            break;
...
esp_err_t RefreshCurrentScreenLocked(RefreshMode refresh_mode)
{
    switch (s_current_screen.load(std::memory_order_relaxed)) {
        ...
        case ScreenId::kSettingsAbout:
            return ApplySettingsAbout(refresh_mode);
        case ScreenId::kWifi:
...
void DisplayTask(void*)
{
    ...
    while (true) {
        ...
        if (command.type == DisplayCommandType::kSetScreen) {
            ...
            } else if (command.screen == ScreenId::kSettingsStorage) {
            ...
            } else if (command.screen == ScreenId::kSettingsAbout) {
                err = ApplySettingsAbout(command.refresh_request.refresh_mode);
...
esp_err_t StartDisplayTask()
{
    ...
    const BaseType_t created = xTaskCreatePinnedToCore(
        ...
        nullptr,
        chroninkle_task_config::kPriorityDisplay,
        &s_display_task,
...
esp_err_t SetSettingsStoragePageState(const epaper_ui::SettingsStoragePageState& state)
{
    ...
    std::lock_guard<std::mutex> lock(s_state_mutex);
    s_settings_storage_page_state = state;
    return ESP_OK;
}
...
esp_err_t SetSettingsTopicsPageState(const epaper_ui::SettingsTopicsPageState& state)
{
    ...
    std::lock_guard<std::mutex> lock(s_state_mutex);
    s_settings_topics_page_state = state;
    return ESP_OK;
}

esp_err_t SetSettingsAboutPageState(const epaper_ui::SettingsAboutPageState& state)
{
    if (!s_initialized) {
        return ESP_ERR_INVALID_STATE;
    }
    ...
    std::lock_guard<std::mutex> lock(s_state_mutex);
    s_settings_about_page_state = state;
    return ESP_OK;
}
