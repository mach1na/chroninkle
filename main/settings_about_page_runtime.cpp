...
namespace settings_about_page_runtime {
namespace {

std::mutex s_mutex;
SettingsAboutPageCoordinator s_coordinator = {};
...
epaper_ui::SettingsAboutPageState BuildStateLocked()
{
...
esp_err_t UpdateDisplayState()
{
    ...
    return display_service::SetSettingsAboutPageState(BuildStateLocked());
}
...
{
    return ui_refresh_runtime::Schedule(ui_refresh_runtime::SurfaceKey::kSettingsAboutPage,
...
page_actions::FocusMoveOutcome MoveFocus(int delta)
{
    ...
    int new_focus_index = -1;
    {
        ...
        result = settings_about_page_interactions::HandleMoveFocus(s_coordinator, delta);
        if (!result.handled) {
            return result;
        }
...
settings_about_page_interactions::ActivateResult ActivateFocusedItem()
{
    std::lock_guard<std::mutex> lock(s_mutex);
    return settings_about_page_interactions::HandlePrimaryActivate(s_coordinator);
}
