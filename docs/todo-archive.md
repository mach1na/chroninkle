...
## ~~Archived todos leaking into Summarize, and stale Home footer highlight~~ — resolved

Two bugs Craig found while testing the page-trio refactor above (not
originally filed as separate todo.md items; noted here for the resolution
history):

1. The Summarize page's Gemini-generated Todos summary included archived
   todos. `components/summary_service/summary_service.cpp`'s
   `FilterWindowedEntries` matched on tag alone (`IsTodoRecordingTag`);
   fixed to also require `!entry.metadata.archived`, matching what the
   Todos page's own Current view already excludes.
2. Entering Notes, Todos, or Follow-up briefly showed the footer's Home
   icon falsely highlighted as selected. Root cause: each `ShowXScreen` in
   `main/app_shell.cpp` built the footer projection from the coordinator's
   focus *before* calling `SyncFromArchive`, which can silently move that
   focus (e.g. a fresh-boot coordinator whose only navigation item is
   Home, or having last left the page via the footer). Fixed by reordering
   each `ShowXScreen` to sync from the archive first, then reset
   focus/build the footer projection.
