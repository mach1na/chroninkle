...
| `captive_dns` | `wifi_service` | 5 | CPU1 | AP-mode captive-portal DNS responder; high priority so a client device's captive-portal probe gets answered promptly. |
...
| `transcribe_retry` | `transcription_retry_service` | 2 | CPU1 | One-shot batch retry of offline-queued transcriptions. Runs on its own task rather than `gemini_service`'s shared worker: it blocks waiting on a transcription result that is itself queued onto that same worker, which would otherwise never get a turn to run. |
...
`components/wifi_service/portal/`, generated by the Vite app in `webserver/`
— see CLAUDE.md's Setup portal section for the build/copy steps), and a
`captive_dns` task (see Task Mapping below) 302-redirects unrecognized
requests to the portal so OS connectivity probes land on it automatically.
...
- full base refresh: used for explicit full refresh requests, wake recovery, the
  ghost-clear flush (see below), and other panel-reset cases

Ghost-clear flush: `EpaperPanel::NeedsGhostingFlush()` turns true after 8
consecutive whole-screen partial refreshes (contrast on unchanged pixels fades
with each one), but doesn't force anything by itself. `display_service::DisplayTask`
polls its command queue with a short timeout (`kGhostingFlushIdleMs`, 500ms)
once a flush is pending; a gap that long with no new command means input has
paused, and that's when the deferred full refresh actually runs — invisible to
active navigation (arrowing through a long list no longer triggers a flash
mid-interaction). A hard ceiling (`kHardPartialRefreshCeiling`, 20) still
forces the flush inline in the panel driver if input never pauses long enough,
so ghosting can't grow unbounded under continuous input.
